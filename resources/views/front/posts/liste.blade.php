...
@section('header') 
@if(Auth::check())
<div class="btn-group pull-right">{!! link_to_route('post.create',
	'Créer un article', [], ['class' => 'btn btn-info']) !!} {!!
	link_to('auth/logout', 'Deconnexion', ['class' => 'btn btn-warning'])
	!!}
</div>
@else {!! link_to('auth/login', 'Se connecter', ['class' => 'btn
btn-info pull-right']) !!} @endif @stop @section('contenu')
@if(isset($info))
<div class="row alert alert-info">{{ $info }}</div>
@endif 
{!! $links !!} @foreach($posts as $post)
